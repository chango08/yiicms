...
(function ($) {
    ...
    $.fn.slickhover = function (options) {
        ...
        var defaults = $.extend({
            animateIn: false,
            icon: '/psicomc/frontend/web/images/plus-black.png',
            color: '#fff',
            opacity: 0.4,
            speed: 400
        }, options);
                ...
                if($(object).is('img')) {
                    if($(object).parent().hasClass('slickhoverWrapper') == false){
                        if(settings.animateIn) {    
                            ...
                            $(object).wrap('<div class="slickhoverWrapper" style="position: relative; max-width:100%;  background:'+settings.color+';"></div>');
                            $(icon).insertBefore($(object));
                        }
                        else
                            $(object).wrap('<div class="slickhoverWrapper" style="position: relative; max-width:100%;  background:url('+settings.icon+') center center no-repeat transparent;"></div>');        
                    }
                }
            },
